#!/bin/sh
while getopts "m:n:i:s:p:u:r:w:t:b:" option;
    do
    case "$option" in
        m ) MODEL=${OPTARG};;
...
az ml model profile -n $PROFILE_NAME -m ${MODEL}:1 --ic $INFERENCE_CONFIG -t myprofileresult.json -w $WORKSPACE -g $RESOURCE_GROUP
